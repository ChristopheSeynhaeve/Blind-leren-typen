...
function movementGuide() {
 ...
 if(movements.length && step===1) {
  const knownMovements=lessons.slice(0,selected+1).flatMap(lesson=>lesson.movements);
  const fingers=movements.filter((movement,index)=>movements.findIndex(other=>other.home===movement.home)===index);
  return `<div class="movement-guide"><strong>Elke lettergroep met dezelfde vinger</strong><div>${fingers.map(m=>`<span><b>${[m.home,...knownMovements.filter(other=>other.home===m.home).map(other=>other.key)].join(' · ').toUpperCase()}</b> ${m.finger}</span>`).join('')}</div><small>Blijf bij elke lettergroep dezelfde vinger gebruiken. Neem rustig de tijd om de beweging te automatiseren.</small></div>`;
 }
