...
import { calculateResult, parseProgress, STORAGE_KEY, type Progress, type Result } from './progress';
...
function exerciseView() {
 ...
 if(finished) return `<button class="text-button" id="back">← Terug naar mijn lessen</button><section class="result-panel"><div class="result-symbol">${icon(finished.passed?'check':'leaf')}</div><span class="eyebrow">LES ${selected+1} · ${e.title}</span><h1>${finished.passed?'Mooi gedaan!':'Elke oefening is vooruitgang.'}</h1><p>${finished.passed?'Deze oefening is afgerond. Je vingers leren steeds meer.':'Je hebt de tekst uitgetypt. Oefen nog eens rustig voor minstens 95% nauwkeurigheid.'}</p><div class="result-stats"><div><strong>${finished.accuracy}%</strong><span>Nauwkeurigheid</span></div><div><strong>${finished.wpm}</strong><span>Woorden per minuut</span></div><div><strong>${finished.seconds}s</strong><span>Geoefend</span></div></div><div class="result-actions"><button id="restart" class="secondary">${icon('repeat')} Opnieuw oefenen</button><button id="next" class="primary">${selected===13&&step===2?'Naar je voortgang':'Volgende oefening'} ${icon('arrow')}</button></div><small>${storageWarning?'Opslaan is niet gelukt.':'Je resultaat is opgeslagen in deze browser.'}</small></section>`;
 return `<div class="exercise-header"><button class="text-button" id="back">← Mijn lessen</button><span>Les ${selected+1} van 14 <span class="muted">/</span> Oefening ${step+1} van 3</span><button class="text-button" id="restart">${icon('repeat')} Opnieuw</button></div><div class="practice-heading"><span class="eyebrow">${l.title.toUpperCase()}</span><h1>${e.title}</h1><p>${l.description}</p></div><section class="typing-card"><div class="typing-toolbar"><span id="typing-status">${icon('keyboard')} Typ de tekst hieronder om te beginnen</span><span><b id="live-accuracy">100%</b> nauwkeurig <i>·</i> <b id="live-time">0:00</b></span></div><div class="typing-area" id="typing-area"><div id="typing-text" class="typing-text" aria-hidden="true"></div><label class="sr-only" for="typing-input">Typ deze tekst: ${e.text}</label><textarea id="typing-input" autocomplete="off" autocapitalize="off" spellcheck="false" aria-describedby="input-help" rows="1"></textarea></div><div class="typing-bottom"><span id="input-help">${paused?'Gepauzeerd. Klik op de tekst om verder te gaan.':'Volg de gemarkeerde toets. Een fout? Probeer dezelfde letter opnieuw.'}</span><span id="character-count">0 / ${e.text.length}</span></div><div class="progress-track"><i id="exercise-progress" style="width:0%"></i></div></section><div class="keyboard-heading"><span>JE TOETSENBORD ALS HOUVAST</span><span><i class="legend-dot"></i> Volgende toets</span></div>${keyboard()}<div class="practice-tip">${icon('info')} Laat je vingers terugkeren naar <strong>Q S D F</strong> en <strong>J K L M</strong>. Kijk naar je scherm.</div>`;
}
function drawTyping() {
 ...
 document.querySelector('#live-accuracy')!.textContent=`${attempts?Math.round(position/attempts*100):100}%`;
...
function bind() {
 ...
 document.querySelector('#next')?.addEventListener('click',()=>{if(selected===13&&step===2){resetExercise();active=false;page='progress';render();}else launch(step===2?selected+1:selected,(step+1)%3);});
